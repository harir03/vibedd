...
// [ARIA] REMOVED: MAF model references
// import { createPolicyModel, getMafModels } from "./actions";
import { createPolicyModel, getAriaModels } from "./actions";
...
export default function PolicyPage() {
    ...
    const fetchModels = async () => {
        // [ARIA] REMOVED: Old MAF model fetch
        // const m = await getMafModels();
        const m = await getAriaModels();
        setModels(m);
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (result.success) {
                // [ARIA] REMOVED: Old MAF model name in alert
                // alert("Policy updated successfully! Ollama model 'aria-policeman' created.");
                alert("Policy updated successfully! Ollama model 'aria-policeman' created.");
                setPolicy("");
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="space-y-2">
                            ...
                            <p className="text-xs text-slate-500 mb-2">
                                {/* [ARIA] REMOVED: MAF model name reference */}
                                Enter the banking security policy below. This will be used to generate the &apos;aria-policeman&apos; model.
                            </p>
                            <textarea
                                id="policy"
                                value={policy}
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6 sm:p-8 h-fit">
                    <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center gap-2">
                            ...
                            {/* [ARIA] REMOVED: MAF branding */}
                            {/* <h2 className="text-lg font-bold text-slate-900">Active MAF Models</h2> */}
                            <h2 className="text-lg font-bold text-slate-900">Active ARIA Models</h2>
                        </div>
                                ...
                                {models.length > 0 ? (
                                    ...
                                    <tr>
                                        <td colSpan={3} className="px-4 py-8 text-center text-slate-400 italic">
                                            {/* [ARIA] REMOVED: MAF reference in empty state */}
                                            {/* No custom models found. */}
                                            No ARIA models found.
                                        </td>
                                    </tr>
                                )}
                            </tbody>
